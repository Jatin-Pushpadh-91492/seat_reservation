...
function App() {
  ...
  useEffect(() => {
    const fetchSeats = async () => {
      try {
        const response = await axios.get('https://seat-reservation-5khq.vercel.app/seats');
  ...
  const handleBooking = async () => {
    try {
      const response = await axios.post('https://seat-reservation-5khq.vercel.app/bookSeats', { seatCount: Number(seatCount) });
  ...
  const handleReinitialize = async () => {
    try {
      const response = await axios.post('https://seat-reservation-5khq.vercel.app/reinitializeSeats');
      setSeats(prevSeats =>
